...
from cane.core.models import Tenant, User, Workspace, Document, SearchLog, ApiKey
from cane.tools.models import AgentLink
from cane.auth.jwt import get_current_user, get_optional_user
from cane.agents.prompts import auto_generate_prompt, generate_replica_prompt
from cane.core.security import sanitize_form_field
from cane.core.config import GUEST_TENANT_ID
from cane.core.limits import check_agent_limit, check_guest_agent_limit
...
def get_agent(agent_id: str, request: Request, user: User = Depends(get_optional_user), db: Session = Depends(get_db)):
    ...
    return {
        ...
        "orchestrator_mode": getattr(ws, "orchestrator_mode", False) or False,
        "document_count": doc_count,
...
def update_agent(
    ...
    if orchestrator_mode is not None:
        ws.orchestrator_mode = orchestrator_mode.lower() in ("true", "1", "yes")
...
def delete_agent(agent_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        # ── 2. Eval cascade ──
        try:
            from cane.eval.models import Environment, TestCase, JudgeCriteria, JudgeCustomRule, EvalRun, EvalResult
        ...
        # ── 3e. Agent communications ──
        _safe_delete(db, "DELETE FROM agent_communications WHERE parent_agent_id = :wid OR child_agent_id = :wid", {"wid": agent_id})
